...
def xml_to_csv(input_xml, output_csv):
    ...
    with open(output_csv, 'w', newline='') as csvfile:
        ...
        csvwriter.writerow(['Host', 'Cipher', 'CipherStrength', 'KeyAlgorithm', 'KeyBits', 'SelfSigned','Serial', 'NotValidBefore', 'NotValidAfter', 'SubjectDN', 'IssuerDN', 'CertificatePEM'])
        ...
        for ssltest in root.findall('.//ssltest'):
            ...
            for certificate in ssltest.findall('.//certificate'):
                ...
                csvwriter.writerow([host, cipher, cipher_strength, key_algorithm, key_bits, self_signed, serial, not_valid_before, not_valid_after, subject_dn, issuer_dn, cert])

                # Print the PEM content for each certificate
                print(f"Certificate PEM for host '{host}':")
                print(cert)

    print("Data has been extracted and saved to", output_csv)
